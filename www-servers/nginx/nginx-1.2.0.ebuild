...
# ngx_chunkin (https://github.com/agentzh/chunkin-nginx-module/, BSD-2 license)
HTTP_CHUNKIN_MODULE_PV="0.22"
HTTP_CHUNKIN_MODULE_P="ngx_chunkin-${HTTP_CHUNKIN_MODULE_PV}"
HTTP_CHUNKIN_MODULE_SHA1="b46dd27"
HTTP_CHUNKIN_MODULE_URI="https://github.com/agentzh/chunkin-nginx-module/tarball/v${HTTP_CHUNKIN_MODULE_PV}"

# nginx_syslog_patch (https://github.com/yaoweibin/nginx_syslog_patch, BSD license)
SYSLOG_MODULE_PV="6f5f90e"
SYSLOG_MODULE_P="ngx_syslog-${SYSLOG_MODULE_PV}"
SYSLOG_MODULE_URI="https://github.com/yaoweibin/nginx_syslog_patch/tarball/${SYSLOG_MODULE_PV}"
...
SRC_URI="http://nginx.org/download/${P}.tar.gz
	...
	nginx_modules_http_chunkin? ( ${HTTP_CHUNKIN_MODULE_URI} -> ${HTTP_CHUNKIN_MODULE_P}.tar.gz )
...
IUSE="aio debug +http +http-cache ipv6 libatomic +pcre pcre-jit ssl syslog vim-syntax"
...
src_prepare() {
	epatch ${WORKDIR}/yaoweibin-nginx_syslog_patch-${SYSLOG_MODULE_PV}/syslog_1.2.0.patch
...
src_configure() {
	...
	# third-party modules
	if use syslog; then
		myconf+=" --add-module=${WORKDIR}/yaoweibin-nginx_syslog_patch-${SYSLOG_MODULE_PV}"
	fi
...
src_install() {
	...
	keepdir /var/www/default/public
	mv "${ED}"/usr/html/* "${ED}"/var/www/default/public || die
